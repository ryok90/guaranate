...
<TerminalWindow title="guaranate — zsh">
  <TerminalFrame name="watch-session" label="A Guaranate watch session: a spinner reading Awake — until the watched process exits, elapsed time, and the watched pid" />
</TerminalWindow>
