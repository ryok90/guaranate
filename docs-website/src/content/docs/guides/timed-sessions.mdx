...
<TerminalWindow title="guaranate — zsh">
  <TerminalFrame name="timed-session" label="A Guaranate timed session at 65%: a green-to-red gradient progress bar above elapsed, remaining, end time, and the assertion mode" />
</TerminalWindow>
...
<TerminalWindow title="guaranate — zsh">
  <TerminalFrame name="completion-card" label="A completed Guaranate session: a full progress bar with a check mark, the time stayed awake, and the assertion released" />
</TerminalWindow>

Both frames are captures of the real binary — text, not screenshots — written
by `npm run gen:frames`.
