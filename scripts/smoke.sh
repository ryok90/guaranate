...
start_line="$(grep -n -m1 "staying awake" "$tostop_out" | cut -d: -f1)"
command_line="$(grep -n -m1 "^COMMAND_RAN" "$tostop_out" | cut -d: -f1)"
(( start_line < command_line )) || fail "the command printed before the start line"
